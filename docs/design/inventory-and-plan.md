...
the marker is present but the deploy-disks sidecar is missing or
unparseable, deploy errors out — otherwise a lost or truncated
sidecar would silently format disks plan deliberately classified
out (excludes, ephemeral, foreign). Hand-written `cluster.yaml`
files (no marker, no sidecars) take the legacy scan-everything
path unchanged.

In addition to the disk-prep filter, deploy refuses to start a
volume server whose SSH target has no plan-approved disks — even
though `weed volume` would happily mkdir its `-dir` paths on the
host's root filesystem, ending up with cluster data on the OS
disk. The check fires before any SSH work so the operator sees a
clear "no plan-approved disks" error pointing at the inconsistency
(stale sidecar, hand edit without `cluster plan --overwrite`).
