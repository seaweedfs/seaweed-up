...
// the presence of its facts.json sibling (always written together).
// When the spec was generated by plan but the deploy-disks sidecar is
// missing, unreadable, or unparseable we return an error rather than
// fall back to deploy's legacy "scan every prefix-matching disk"
// behavior — that fallback would silently format disks the planner
// deliberately excluded.
//
// (nil, nil) means "hand-written cluster.yaml; no sidecars; legacy
// behavior is correct". (map, nil) carries the allowlist (possibly
// empty for hosts plan classified zero disks for). (nil, err) means
// "plan-generated but sidecar is broken — refuse to deploy".
func loadPlannedDeployDisks(specPath string) (map[string]map[string]struct{}, error) {
	sidecar := deployDisksFilePath(specPath)
	planGenerated := fileExists(factsFilePath(specPath))
	sidecarExists := fileExists(sidecar)

	if !planGenerated && !sidecarExists {
		// Hand-written cluster.yaml — no plan, no allowlist, legacy
		// scan-everything behavior is intentional.
		return nil, nil
	}
	if !sidecarExists {
		return nil, fmt.Errorf(
			"%s exists (cluster.yaml looks plan-generated) but %s is missing — "+
			...
			factsFilePath(specPath), sidecar, specPath)
	}
	...
	// The map can legitimately be empty (e.g., a cluster with no
	// volume hosts), so don't treat that as missing. Always return a
	// non-nil map when the sidecar parsed cleanly so the manager
	// applies the (possibly empty) allowlist authoritatively rather
	// than falling back.
